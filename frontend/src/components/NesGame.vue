<template>
  <div style="width: 100%; height: auto">
    // TODO select
    <div>
      <label><input type="text" placeholder="rom url" v-model="romUrl"/></label>
      <button @click="nes_load_url()">load</button>
      <p>DPad: Arrow keys<br/>Start: Return, Select: Tab<br/>A Button: A, B Button: S</p>
    </div>
    <div style="margin: auto;width: 80%; height: auto">
      <canvas ref="game" id="nes-game" width="256" height="240" style="width:100%"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      romUrl: "roms/eat_beans.nes",
    };
  },
  methods: {
    nes_load_url: function () {
      nes_init("nes-game");
      var req = new XMLHttpRequest();
      req.open("GET", this.romUrl);
      req.overrideMimeType("text/plain; charset=x-user-defined");
      req.onerror = () => console.log(`Error loading ${this.romUrl}: ${req.statusText}`);
      req.onload = function () {
        if (this.status === 200) {
